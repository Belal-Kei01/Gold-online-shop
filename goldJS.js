...
const historyDateElem = document.querySelector(".historyDate");
window.addEventListener("load", (event) => {
  event.preventDefault();
  fetch(`https://brsapi.ir/Api/Market/Sample/FreeApi_Gold_Currency.json`, {
    method: "GET",
  })
    .then((resolve) => {
    ...
    .then((date) => {
      ...
      goldPriceAtMoment.innerHTML = ` قیمت طلا :  ${date.gold[0].price}`;
      historyDateElem.innerHTML = `تاریخ روز : ${date.gold[0].date}`;
      return date.price;
    })
    .catch((err) => {
